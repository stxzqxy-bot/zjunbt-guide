...
dormGrid.innerHTML=dorms.map(d=>{const imgs=(d.images||[]).slice(0,3).map((src,i)=>`<img src="${src}" alt="${d.name}图片${i+1}">`).join('');const details=(d.details||[]).map(x=>`<li>${x}</li>`).join('');return `<article class="dorm-card" style="--card:${d.color}"><div class="dorm-gallery">${imgs}</div><span class="status">● 已补充资料</span><h3>${d.name}</h3><p>${d.meta}</p><ul>${details}</ul></article>`}).join('');
...
const dialog=document.querySelector('#askDialog');document.querySelectorAll('[data-open-ask]').forEach(b=>b.onclick=()=>dialog.showModal());document.querySelector('#askForm').addEventListener('submit',e=>{e.preventDefault();const text=document.querySelector('#askText').value.trim();if(!text){return}const saved=JSON.parse(localStorage.getItem('nbt-questions')||'[]');saved.push({text,category:document.querySelector('#askCategory').value,time:new Date().toISOString()});localStorage.setItem('nbt-questions',JSON.stringify(saved));document.querySelector('#askText').value='';dialog.close();const toast=document.querySelector('#toast');toast.textContent='问题已保存在本机，感谢你参与共建';toast.classList.add('show');setTimeout(()=>toast.classList.remove('show'),2600)});
