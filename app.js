...
async function copyText(text){try{await navigator.clipboard.writeText(text);return true}catch(e){const ta=document.createElement('textarea');ta.value=text;ta.setAttribute('readonly','');ta.style.position='fixed';ta.style.opacity='0';document.body.appendChild(ta);ta.select();const ok=document.execCommand('copy');ta.remove();return ok}}
document.querySelectorAll('[data-copy-mini]').forEach(btn=>btn.addEventListener('click',async()=>{const text=btn.dataset.copyMini;const ok=await copyText(text);const toast=document.querySelector('#toast');toast.textContent=ok?'已复制校园全景 VR 小程序入口，请到微信中打开':'复制失败，请手动复制小程序入口';toast.classList.add('show');setTimeout(()=>toast.classList.remove('show'),2600)}));
